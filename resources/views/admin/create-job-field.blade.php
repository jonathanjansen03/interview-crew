...
@section('main-content')
    ...
    <main class="relative flex justify-center items-center">
        <div class="flex flex-col justify-center absolute bg-white drop-shadow-lg inset-y-48 w-1/4 h-fit py-5 px-6 rounded-xl">
            <h3 class="text-black font-bold text-2xl">Create Job Field</h3>

            <form action="/create-job-field" method="POST" enctype="multipart/form-data" class="flex flex-col mt-5">
                @csrf
                <label for="field" class="font-medium">Job Field Name</label>
                <input type="text" id="field" name="field" class="border px-2 py-1 rounded-lg bg-slate-200 focus:border-sky-500 focus:outline-none">
